    ...
    try {
      const res = await fetch("http://localhost:6868/api/v1/contacts", {
        ...
        body: JSON.stringify({
          ...
          email,
          message,
          reply: "Đang xử lý", // Bạn có thể đặt giá trị mặc định
          status: "Chưa phản hồi", // Giá trị mặc định
        }),
      });
      ...
      if (res.ok && data.status === "success") {
        setStatusMsg("Phản hồi của bạn đã được gửi thành công.");
        setName("");
        setEmail("");
        setMessage("");
      } else {
        setStatusMsg("Gửi phản hồi thất bại.");
      }
    } catch (err) {
      console.error("Lỗi gửi:", err);
      setStatusMsg("Đã xảy ra lỗi.");
    }
    ...
    <main className="bg-white min-h-screen">
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="text-center mb-8">
          ...
          <div className="w-16 h-1 bg-[#f75757] mx-auto mb-4 rounded"></div>
          <p className="text-[#6f8ba4] max-w-xl mx-auto">
            Gửi phản hồi cho chúng tôi, đội ngũ sẽ liên hệ lại sớm nhất có thể!
          </p>
        </div>

        <form onSubmit={handleSubmit} className="bg-[#f4f8fb] rounded-lg p-6 md:p-8">
          {statusMsg && (
            <p className="text-center mb-4 text-sm text-blue-600">{statusMsg}</p>
          )}
