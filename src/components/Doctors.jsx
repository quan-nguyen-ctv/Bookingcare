  ...
  useEffect(() => {
    const fetchSpecialties = async () => {
      try {
        ...
        const json = await res.json();
        const list = json?.data?.specialtyList;
        if (!Array.isArray(list)) throw new Error("Không thể tải chuyên khoa");
        setSpecialties(list);
        if (list.length > 0) {
          setActiveSpecialtyId(list[0].id);
        }
      } catch (err) {
        setError("Lỗi tải chuyên khoa: " + err.message);
      }
    };
    fetchSpecialties();
  }, []);

  // Gọi API lấy bác sĩ theo chuyên khoa
  useEffect(() => {
    if (!activeSpecialtyId) return;
    const fetchDoctors = async () => {
      try {
        ...
        const json = await res.json();
        const list = json?.data?.doctors;
        if (!Array.isArray(list)) throw new Error("Không thể tải bác sĩ");
        setDoctors(list);
      } catch (err) {
        setError("Lỗi tải bác sĩ: " + err.message);
      }
    };
    fetchDoctors();
  }, [activeSpecialtyId]);
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
            ...
            {doctors.map((doctor) => (
              <div key={doctor.id} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition duration-300 group">
                ...
                <div className="p-6 text-center">
                  ...
                  <p className="text-gray-600 text-sm mb-4">
                    Bác sĩ giàu kinh nghiệm trong lĩnh vực chăm sóc sức khỏe
                  </p>
          ...
          {doctors.length === 0 && !error && (
            <div className="text-center py-16">
              <FaUserMd className="text-6xl text-gray-300 mx-auto mb-4" />
              <h3 className="text-xl font-semibold text-gray-500 mb-2">Chưa có bác sĩ</h3>
              <p className="text-gray-400">Vui lòng chọn chuyên khoa khác hoặc thử lại sau.</p>
            </div>
          )}
        </div>
      </section>
