...
const MedicalServiceDetail = () => {
  ...
  const [schedules, setSchedules] = useState([]); // Thêm state cho schedules
  ...
  useEffect(() => {
    ...
    // Gọi API schedules chỉ với specialtyId
      ...
      <section className="container mx-auto px-4 py-10">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 bg-white rounded-xl shadow p-6">
            ...
            <h3 className="text-xl font-bold text-[#223a66] mb-4">{specialty?.specialtyName}</h3>
          ...
          {/* Right: Schedule & Address */}
          <div className="w-full lg:w-96 bg-white rounded-xl shadow p-6">
            <h5 className="text-lg font-semibold mb-4 text-[#223a66]">Time Schedule</h5>
            <ul className="mb-4">
            ...
            <div className="mt-4">
              ...
              {clinics.length > 0 ? (
                clinics.map((clinic) => (
                  <p key={clinic.id} className="text-gray-700 text-sm">- {clinic?.address}</p>
                ))
              ) : (
        ...
        {/* Doctors */}
        <div className="mt-10">
          <h3 className="text-2xl font-bold text-[#223a66] mb-4">Doctors in this Department</h3>
          <div className="border-b mb-6"></div>
          <div className="grid grid-cols-1 gap-6">
            {doctors.length > 0 ? (
              doctors.map((doctor) => (
                <div key={doctor?.id} className="bg-white rounded-xl shadow p-6 flex flex-col md:flex-row gap-6">
                  <div className="flex flex-col md:flex-row gap-4 flex-1">
                    ...
                    />
                    <div>
                      <h4 className="text-lg font-bold text-[#223a66]">{doctor?.user?.fullname}</h4>
                      ...
                      {doctor?.qualification && (
                        <p className="text-gray-600 text-sm">Qualification: {doctor.qualification}</p>
                      )}
                      <button
                  ...
                  <div className="md:w-1/2 mt-4 md:mt-0">
                    <h5 className="font-semibold mb-2 text-[#223a66]">Schedule:</h5>
                    {/* <ScheduleDoctorLayout idDoctor={doctor?.id} /> */}
                    <span className="text-gray-400 text-sm">[Lịch khám sẽ hiển thị ở đây]</span>
                  </div>
                </div>
              ))
            ) : (
